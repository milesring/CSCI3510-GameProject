...
public class CharacterSpawner : MonoBehaviour {
	...
	public void Init(){
		...
		float randz = 0f;
		float height = 0f;
		float buffer = groundSize.x/4;
		GameObject enemy = (GameObject)Resources.Load ("Prefabs/Characters/Enemy");
		for (int i = 0; i < enemyCount; ++i) {
			randx = Random.Range (-groundSize.x / 2 + buffer, groundSize.x / 2 - buffer);
			randz = Random.Range (-groundSize.z / 2 + buffer, groundSize.z / 2 - buffer);
